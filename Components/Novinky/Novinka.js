...
export default function Novinka(props){
    ...
    return(
       ...
       <Text weight={700} size="xl" sx={{ color: "#545454", fontSize: "1.5rem", marginRight: "auto", marginLeft: device !== "laptop" ? "5vw" : "1vw"}}>{item.attributes.Nadpis}</Text>
        ...
        {item.attributes.Obrazek.data && <img alt={item.attributes.Obrazek.data.attributes.alternativeText} src={"http://localhost:1337" + item.attributes.Obrazek.data.attributes.formats.small.url} style={{ width: "100%", height: "80%", objectFit: "contain", maxHeight: "30vh" }} height={item.attributes.Obrazek.data.attributes.formats.small.height} width={item.attributes.Obrazek.data.attributes.formats.small.width}/>}
       ...
       <div style={{display: "flex", alignItems: "center", margin: "1vh 1vw", maxWidth: "100%", flexWrap: "wrap", gap: "2vw"}}>
        {isShown && item.attributes.Priloha.data && item.attributes.Priloha.data.map(priloha => {
            console.log(priloha)
            return(<Badge color="lime"><Text key={priloha.id} component="p" weight={400} size="xl" sx={{ color: "#545454", fontSize: "0.8rem", textAlign: "center", margin: "0" }}><a style={{color: "#545454"}} href={"http://localhost:1337" + priloha.attributes.url} download={"http://localhost:1337" + priloha.attributes.url} >{priloha.attributes.name}</a></Text></Badge>)
        })}
        </div>
        </div>
        </Paper>
